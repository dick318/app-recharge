<template>
  <div class="wrap cardInfoPage" v-bind:class="[logoClass]">
    <div class="cardInfo">
      <div class="basicInfo clearfix">
        ...
        <div class="basicInfoRight">
          <ul class="list">
            ...
            <li class="daoqi">
              账户余额：
              <span id="lastMoney" style="font-size:16px;">{{amount}}</span>元
            </li>
            <li>
              卡状态：
              <span id="cardStatus">{{cardstatus}}</span>
            </li>
            <li class="daoqi daoqi_">
              到期日期:
              <span>{{endtime}}</span>
            </li>
          </ul>
      ...
      <div class="fluxInfo">
        ...
        <div class="clearfix line1" v-bind:class="[addClass]" v-if="time == 'show'">
          ...
          <span class="fr" v-if="endShowTime =='show'">已过期</span>
          <span class="fr" v-else id="colockbox1">
            <span>
    ...
    <div class="goPage" @click="order" v-if="showStatus">
    ...
    get_info: function() {
      ...
      c.$post2(
        ...
        function(res) {
          _this.shuaxin = "";
          if (res.status == 0) {
            _this.getCardStatus(res);
            if (res.data.infiniteTime) {
              _this.getInfinite(res);
            } else {
              _this.flow = "show";
            }
            _this.ptime = "show";
