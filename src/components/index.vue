<template>
  <div
    id="bindCardPage"
    v-bind:class='[moreClass,baseClass]'
  >
    <div class="logo">
      <img
        id="logo_img"
    ...
    <div class="bindCardMain">
      ...
      >
        <ul class="list">
          <li
            v-for='cardNo in cards'
            @click='getCard(cardNo)'
            :key="cardNo"
          >
            {{ cardNo }}
             <i
              class="weui-icon-cancel"
              @click.stop="bindCardDel(cardNo)"
            ></i>
          </li>
        </ul>
      </div>
...
#bindCard {
  ...
  text-align: center;
  border-radius: 4px;
  background: -webkit-linear-gradient(left, #fcf0b3, #ffdf71);
}
</style>
...
export default {
      ...
      if (!cardNo) {
        ...
        if (sessionStorage.getItem("Alipay") == "true") {
          ...
          if (localStorage.getItem("code") == code) {
            url = c.urlPrefix + "?r=wms/personal/card-bind";
            data = {
              card: cardNo,
              uid: localStorage.getItem("id")
            };
          } else {
            url = c.urlPrefix + "?r=wms/personal/alipay-card-bind";
            data = {
              card: cardNo,
              uid: localStorage.getItem("id"),
              code: code
          ...
          url = c.javaWt + "client/v3/loginInfo";
          data = {
            card: cardNo,
            uid: localStorage.getItem("id")
          };
        ...
        c.$post2(url, data, function(res) {
          if (res.status == 0 || res.code == 1) {
            ...
            if (_this.aid && _this.aid != res.data.fid) {
            ...
            if (localStorage.getItem("localStorageCard")) {
              const localStorageCard = localStorage
                ...
                .split(",");
              if (!localStorageCard.find(n => n == cardNo)) {
                localStorageCard.unshift(cardNo);
              }
              localStorage.setItem(
                "localStorageCard",
                localStorageCard.join(",")
              );
            } else {
              localStorage.setItem("localStorageCard", cardNo);
            }
            ...
            if (res.data.remarks == 7 || res.data.remarks == "换卡" || res.data.remarks == "12") {
              $.alert(
            ...
            switch (+res.data.zid) {
              ...
              case 216:
                window.location.href =
                  "https://wx.china-m2m.com/weixin/new_auth";
                return;
              ...
              case 217:
                if (res.data.operatorType == 1 || res.data.operator_type == 1) {
                  ...
                  window.location.href =
                    "http://www.szhuazheng.cn/cardInfo.html";
                } else if (
  ...
  beforeCreate: function() {
    ...
    if (navigator.userAgent.indexOf("Alipay") > -1) {
      ...
      if (window.location.search.indexOf("code") == -1) {
        // 转授权链接
        let data = {};
        if (sessionStorage.getItem('otherAid')=='true') {
          data = {
            url: document.location.href,
            aid: _this.$route.query.aid || sessionStorage.getItem("aid"),
        ...
        c.$post2(c.javaWt + "client/v3/wxCodeUrl", data, function(res) {
          if (res.status != 0) {
            $.toptip(res.message, 2000, "error");
            return;
          }
  ...
  created: function() {
    this.otherAid = sessionStorage.getItem('otherAid')=='true'
    try {
      this.imgUrl = require(`../img/${this.$route.query.aid}logo.png`);
    } catch (err) {
      if(this.otherAid){
        this.imgUrl = require(`../img/5862logo.png`);
      }else{
        this.imgUrl = "";
      }
    }
      ...
      if (!sessionStorage.getItem("openid")) {
        ...
        if (this.otherAid) {
          if (sessionStorage.getItem("needFresh") != "true") {
            data = {
              uid: _this.$route.query.id || localStorage.getItem("id"),
              aid: _this.$route.query.aid || localStorage.getItem("aid"),
          ...
          c.$post2(c.javaWt + "client/v3/openid", data, function(res) {
            if (res.status == 0) {
