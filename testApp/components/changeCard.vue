<template>
  <div>
    <van-notice-bar
      text="春节前后，由于大部分快递停止发送，我们将在年后发货，如果一定需要发送的请联系客服（1月27日前），我们也可以为您顺丰快递货到付款。祝大家春节愉快，阖家幸福。"
      left-icon="volume-o"
    />
...
export default {
  ...
  methods: {
    ...
    onBridgeReady: function(data) {
      ...
      WeixinJSBridge.invoke("getBrandWCPayRequest", data, function(res) {
        if (res.err_msg == "get_brand_wcpay_request:ok") {
          _this.$router.push({ path: "changeCardList" });
        } else {
          $.toptip(res.message, 2000, "error");
        }
      });
    ...
    add: function() {
      ...
      $.confirm({
        text:
          "春节前后，由于大部分快递停止发送，我们将在年后发货，如果一定需要发送的请联系客服（1月27日前），我们也可以为您顺丰快递货到付款。祝大家春节愉快，阖家幸福。",
        title: "注意",
        onOK: function() {
          c.$post2(
